...
def create_context_message(context_text, tone=""):
    ...
    if tone:
        base_prompt += f"Hãy trả lời với tone giọng {tone}"
    if context_text:
        base_prompt += f"Tham khảo những thông tin sau đây khi trả lời:\n\n{context_text}\n\nHãy trả lời thông tin dựa vào context người dùng cho sẵn để trả lời câu hỏi của người dùng một cách chính xác và chi tiết."
    return base_prompt
...
def initialize_openai():
    ...
    if not api_key:
        st.error("Vui lòng cung cấp khóa API OpenAI trong biến môi trường OPEN_API_KEY.")
        st.stop()
